      ...
      <el-form :model="form" inline label-width="200px" ref="form">
        ...
        <div>
          <el-form-item label="技能名：" prop="name" :rules="$genRules({rule:/.+/,message:'技能名输入有误'})">
            <el-input v-model="form.name" placeholder="请输入技能名" class="bz-width-400"/>
          </el-form-item>
        </div>
        ...
        <div>
          <el-form-item label="技能简介：" prop="description" :rules="$genRules({rule:/.+/,message:'技能名输入有误'})">
            <el-input v-model="form.description" placeholder="请输入技能简介" class="bz-width-400"/>
          </el-form-item>
        </div>
        ...
        <div>
          <el-form-item label="等级：" prop="level" :rules="$genRules({rule:/^\d+$/,message:'等级输入有误'})">
            <el-input v-model.number="form.level" placeholder="请输入等级" class="bz-width-400"/>
          </el-form-item>
        </div>
        ...
        <div>
          <el-form-item label="消耗hp：" prop="consumeHp" :rules="$genRules({rule:/^\d+$/,message:'消耗hp输入有误'})">
            <el-input v-model.number="form.consumeHp" placeholder="请输入消耗hp" class="bz-width-400"/>
          </el-form-item>
        </div>

        <div>
          <el-form-item label="消耗mp：" prop="consumeMp" :rules="$genRules({rule:/^\d+$/,message:'消耗mp输入有误'})">
            <el-input v-model.number="form.consumeMp" placeholder="请输入消耗mp" class="bz-width-400"/>
          </el-form-item>
        </div>
        ...
        <div>
          <el-form-item label="增益：" prop="isGain" :rules="$genRules({rule:/^\d+$/,message:'请选择增益'})">
            <SelectConfigType name="TRUE_OR_FALSE_TYPE" v-model="form.isGain" class="bz-width-400"
                              @change="changeGain"/>
          </el-form-item>
        </div>
        ...
        <div v-if="form.isGain === 1">
          <el-form-item label="增益时效时间：" prop="duration" :rules="$genRules({rule:/^\d+$/,message:'增益时效时间输入有误'})">
            <el-input v-model.number="form.duration" placeholder="请输入增益时效时间" class="bz-width-400"/>
          </el-form-item>
        </div>
        ...
        <div v-if="form.isGain === 1">
          <el-form-item label="hp增益：" prop="hpGain" :rules="$genRules({rule:/^\d+$/,message:'hp增益输入有误'})">
            <el-input v-model.number="form.hpGain" placeholder="请输入hp增益" class="bz-width-400"/>
          </el-form-item>
        </div>
        ...
        <div v-if="form.isGain === 1">
          <el-form-item label="mp增益：" prop="mpGain" :rules="$genRules({rule:/^\d+$/,message:'mp增益输入有误'})">
            <el-input v-model.number="form.mpGain" placeholder="请输入mp增益" class="bz-width-400"/>
          </el-form-item>
        </div>
        ...
        <div>
          <el-form-item label="物理攻击：" prop="physicalAttack" :rules="$genRules({rule:/^\d+$/,message:'物理攻击输入有误'})">
            <el-input v-model.number="form.physicalAttack" placeholder="请输入物理攻击" class="bz-width-400"/>
          </el-form-item>
        </div>
        ...
        <div v-if="form.isGain === 1">
          <el-form-item label="物理攻击增益：" prop="physicalAttackGain"
            ...
            <el-input v-model.number="form.physicalAttackGain" placeholder="请输入物理攻击增益" class="bz-width-400"/>
          </el-form-item>
        </div>
        ...
        <div v-if="form.isGain === 1">
          <el-form-item label="物理防御增益：" prop="physicalDefenseGain"
            ...
            <el-input v-model.number="form.physicalDefenseGain" placeholder="请输入物理防御增益" class="bz-width-400"/>
          </el-form-item>
        </div>
        ...
        <div>
          <el-form-item label="魔法攻击：" prop="magicAttack" :rules="$genRules({rule:/^\d+$/,message:'魔法攻击输入有误'})">
            <el-input v-model.number="form.magicAttack" placeholder="请输入魔法攻击" class="bz-width-400"/>
          </el-form-item>
        </div>

        <div v-if="form.isGain === 1">
          <el-form-item label="魔法攻击增益：" prop="magicAttackGain" :rules="$genRules({rule:/^\d+$/,message:'魔法攻击增益输入有误'})">
            <el-input v-model.number="form.magicAttackGain" placeholder="请输入魔法攻击增益" class="bz-width-400"/>
          </el-form-item>
        </div>
        ...
        <div v-if="form.isGain === 1">
          <el-form-item label="魔法防御增益：" prop="magicDefenseGain" :rules="$genRules({rule:/^\d+$/,message:'魔法防御增益输入有误'})">
            <el-input v-model.number="form.magicDefenseGain" placeholder="请输入魔法防御增益" class="bz-width-400"/>
          </el-form-item>
        </div>
        ...
        <div v-if="form.isGain === 1">
          <el-form-item label="暴击率增益：" prop="critGain" :rules="$genRules({rule:/^\d+$/,message:'暴击率输入有误'})">
            <el-input v-model.number="form.critGain" placeholder="请输入暴击率" class="bz-width-400"/>
          </el-form-item>
        </div>
        ...
        <div v-if="form.isGain === 1">
          <el-form-item label="命中率增益：" prop="hitGain" :rules="$genRules({rule:/^\d+$/,message:'命中率输入有误'})">
            <el-input v-model.number="form.hitGain" placeholder="请输入命中率" class="bz-width-400"/>
          </el-form-item>
        </div>
        ...
        <div v-if="form.isGain === 1">
          <el-form-item label="闪避率增益：" prop="dodgeGain" :rules="$genRules({rule:/^\d+$/,message:'闪避率输入有误'})">
            <el-input v-model.number="form.dodgeGain" placeholder="请输入闪避率" class="bz-width-400"/>
          </el-form-item>
        </div>
        ...
        <div v-if="form.isGain === 1">
          <el-form-item label="暴抗率增益：" prop="critResistanceGain" :rules="$genRules({rule:/^\d+$/,message:'暴抗率输入有误'})">
            <el-input v-model.number="form.critResistanceGain" placeholder="请输入暴抗率" class="bz-width-400"/>
          </el-form-item>
        </div>
  ...
  export default {
    ...
    methods: {
      ...
      getInfo() {
        ...
        this.$axios.get(`/scene/skill/info`, {
          params: {
            id: query.id
          }
        }).then((res) => {
